...
namespace Cedille2.Proof
  ...
  case lam A' K1 K2 B' t j1 j2 j3 _ _ _ => sorry
    -- intro m C A b B h1 h2 h3; subst h1; subst h2
  ...
  @[simp]
  abbrev InterDestructLemmaType (Γ : Ctx Term) : (v : JudgmentVariant) -> JudgmentIndex v -> Prop
  | .prf => λ (t, T) => ∀ g1 g2 D c d A B, t = .inter g1 g2 D c d -> T =β= [A]∩ B ->
    (∃ C, A =β= C ∧ Γ ⊢ c : C ∧ Γ ⊢ C : ★)
    ∧ (∃ D, B =β= D ∧ Γ ⊢ d : D ∧ Γ ⊢ D : ★)
  | .wf => λ () => True

  theorem inter_destruct_lemma : Judgment v Γ ix -> InterDestructLemmaType Γ v ix := by
  intro j; induction j <;> simp at *
  case inter Γ a A b B j1 j2 j3 j4 _ _ _ _ => sorry
  ...
  | nil : CtxRed [] []
  | head {A : Term} : A -β> A' -> CtxRed Γ Γ' -> CtxRed (A::Γ) (A'::Γ')
  ...
  case app Γ f A B a B' j1 j2 j3 ih1 ih2 => sorry
    ...
    --         apply Red.Conv.refl
  case inter_ty Γ A B j1 j2 ih1 ih2 => sorry
    -- intro t' Γ' r1 r2
    -- cases r1
    -- case _ r1 =>
    --   rw [<-r1]; constructor; apply ih1 (Or.inl rfl) r2
  ...
  case inter Γ a A b B j1 j2 j3 j4 ih1 ih2 ih3 ih4 => sorry
    -- intro t' Γ' r1 r2
    -- cases r1
    -- case _ r1 =>
    --   rw [<-r1]; constructor; apply ih1 (Or.inl rfl) r2
