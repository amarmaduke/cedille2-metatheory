...
namespace Cedille
  ...
  @[simp] lemma erase_fst : erase (fst t) = erase t := by generalize t = s; rfl
  @[simp] lemma erase_fst_unfolded
    : erase (Syntax.ctor Constructor.fst t1 t2 t3) =erase t1
  := rfl

  @[simp] lemma erase_snd : erase (snd t) = erase t := by generalize t = s; rfl
  ...
  lemma erase_rename : erase t1 = erase t2 -> erase ({_|-> x}{_<-| y}t1) = erase ({_|-> x}{_<-| y}t2)
  := by sorry

  lemma erase_free_rename :
    ((x : Name) -> x ∉ fv t -> x ∉ (fv ∘ erase) ({_|-> x}t)) ->
    (x y : Name) -> x ∉ fv t -> y ∉ fv t -> erase ({_|-> x}t) = erase ({_|-> y}t)
  := by {
