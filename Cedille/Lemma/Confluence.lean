...
namespace Cedille
  ...
  inductive ParRed {n : Nat} : Term n -> Term n -> Prop where
  | beta {S : FvSet!} :
    ParRed t1 t1' ->
    (∀ x, x ∉ S -> ParRed ({_|-> x}t2) ({_|-> x}t2')) ->
    ParRed t3 t3' ->
    ParRed (app m (lam m t1 t2) t3) ([_:= t3']t2')
  | fst :
    ParRed t1 t1' ->
    ParRed t2 t2' ->
    ParRed t3 t3' ->
    ParRed (fst (pair t1 t2 t3)) t1'
  | snd :
    ParRed t1 t1' ->
    ParRed t2 t2' ->
    ParRed t3 t3' ->
    ParRed (snd (pair t1 t2 t3)) t2'
  | eqind :
    ParRed t1 t1' ->
    ParRed t2 t2' ->
    ParRed t3 t3' ->
  ...
  | promote :
    ParRed t t' ->
    ParRed (promote (refl (fst t))) (refl t')
  | phi :
    ParRed t1 t1' ->
    ParRed t2 t2' ->
    ParRed t3 t3' ->
    ParRed (fst (phi t1 t2 t3)) t1'
  | bind {S : FvSet!} :
    ParRed t1 t1' ->
    ((x : Name) -> x ∉ S -> ParRed ({_|-> x}t2) ({_|-> x}t2')) ->
  ...
  namespace ParRed
    ...
    lemma refl_bind_open (x : Name) : x ∉ fv t -> ParRed t t -> ParRed ({_|-> x}t) ({_|-> x}t) := by {
      sorry
    }
    ...
    lemma refl_n : ParRed t t := by {
      ...
      case bind n1 n2 b t1 t2 ih1 ih2 => {
        apply ParRed.bind; apply ih1
        swap; exact (fv t2)
        intro x xn
        apply refl_bind_open x xn ih2
      }
      case ctor n1 n2 k t1 t2 t3 ih1 ih2 ih3 => {
  ...
  lemma red_implies_par_step : t -β> s -> t -p> s := by {
    ...
    case beta m t1 t2 t3 => {
      ...
      intro x _; apply ParRed.refl
      apply ParRed.refl; exact []
    }
    case fst => {
      apply ParRed.fst
    ...
    case bind1 t1 t1' k t2 _ ih => {
      apply ParRed.bind; exact ih
      intro x _; apply ParRed.refl; exact []
    }
    case bind2 t2 t2' k t1 S _ ih => {
  ...
  @[simp] lemma parp_kindu {n} : parp kindu = @kindu n := by {
    unfold parp; unfold kindu; simp
  }

  lemma parp_open {n} (t : Term (n + 1)) : {_|-> x}parp t = parp ({_|-> x}t) := by {
    sorry
  }

  lemma par_subst (S : FvSet!) :
    t -p> t' ->
    (∀ x, x ∉ S -> {_|-> x}s -p> {_|-> x}s') ->
    [_:= t]s -p> [_:= t']s'
  := by {
