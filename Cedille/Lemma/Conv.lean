...
namespace Cedille.Conv
  ...
  lemma refl : A === A := Conv.conv RedStar.refl RedStar.refl BetaConv.refl

  lemma symm : A === B -> B === A := by {
    intros e; cases e; case _ u1 u2 r1 e r2 =>
    apply Conv.conv r2 r1 _
    apply BetaConv.symm e
  }

  lemma by_beta : erase a =β= erase b -> a === b := by {
    intro h
  ...
  := by {
    ...
    have lem2 := preservation_of_pseobj h r1
  ...
  := by {
    ...
    have h2 : PseObj u2 := preservation_of_pseobj h r2
    have h3 : PseObj v1 := preservation_of_pseobj h r3
    replace e1 := erase_red_f h2 (BetaConv.symm e1) rr1
    replace e2 := erase_red_f h3 e2 rr2
    have lem := BetaConv.trans (BetaConv.symm e1) e2
    exact lem
  }

  lemma to_beta (S : FvSet!) : PseObj a -> PseObj b -> a === b -> erase a =β= erase b
  := by {
    intro h1 h2 cv
    cases cv; case _ a' b' r1 e r2 =>
    replace e := erase_red_b h1 e r1
    replace e := erase_red_b h2 (BetaConv.symm e) r2
    apply BetaConv.symm e
  }
